...
void Parallelize::updateWeights(double learningRate, double *centers, double *target, double *output, double *weights, int centerSize, int targetSize){
	  ...
	  for (int i = 0; i < centerSize; i++){
		  weights[i] = learningRate*(target[lround(centers[i])]-output[lround(centers[i])]);
...
__global__ void d_calcOutput(double *weights, double *kernels, double *output, int centerSize, int targetSize){
	...
	if(i < targetSize){
		...
		for(int j = 0; j < centerSize; j++){
				value += kernels[(j*targetSize+1+i)]*weights[j];
		}
		output[i]=value;
	}
}
...
void d_deltarule(double learningRate, double *centers, double *target, double *output, double *weights, double *kernels, int centerSize, int targetSize, int kernelSize, int iterations){
...
std::vector<double> Parallelize::applyDeltaRule(double learningRate, std::vector<double> centers, std::vector<double> target, std::vector<double> weights, std::vector<double> kernels, int learningIterations){
	...
	d_deltarule(learningRate, centersArr, targetArr, outputArr, weightsArr, kernelsArr, centers.size(), target.size(), kernels.size(), learningIterations);
